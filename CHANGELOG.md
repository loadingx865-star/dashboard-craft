...
### 变更
- 移除 `tests/`、`tools/`、`.github/`：这些是仓库自身的开发验证脚手架，不属于 Skill 运行时资产，也不会被 `npx skills add` 安装到使用方。仓库现只收录 Skill 本体与面向人的文档
- README 语言切换由 shields.io 徽章改为纯文本链接（中文页为：简体中文 | [English](README.en.md)），避免徽章在部分网络环境下加载不出导致切换入口不可用
- `CONTRIBUTING.md` 去掉对已移除脚手架的依赖，改为官方 spec 校验 + 脚本可启动性检查

### 说明
- 本次不改动 Skill 本体：`SKILL.md`、`references/`、`assets/`、`scripts/` 行为与之前一致，已安装用户无需重装
